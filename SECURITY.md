...
- Create/validate/stop/wake **and every `eval` command except `help`** refuse insecure data directories: on Unix, symlink / wrong owner / group/world-writable (stop fails open to `{}`; wake arm/loop/tick refuse; eval refuses). On Windows, symlink / junction / reparse point (same refuse behavior). Symlink/reparse checks inspect the **unresolved** configured path (including ancestors) so `Path.resolve()` cannot bypass the trust boundary. Probe `OSError` fails closed (treat as insecure). Stop singleflight acquires only after the same insecure/ACL refuse gates. `manage status` and `manage doctor` still **read** an insecure data dir so they can report FAIL.
- On Windows, create/validate/**stop/wake**/eval commands refuse when ACL harden was attempted and failed (doctor also hard-fails). Inheritance strip failure is a hard fail (path is not marked hardened). If inheritance was stripped and the grant fails, inheritance is restored (`icacls /inheritance:e`) best-effort before recording the failure. `CURSOR_GOAL_SKIP_ACL=1` is **test/emergency-only** after manually locking down the path.
...
- Secret-ish tokens in validation commands **and validation output** / status reasons **and goal conditions** are redacted (heuristic; incomplete by design) on these surfaces: `manage status` / create conflict messages / logs, `manage doctor` goal summary, `eval prompt`, `eval validate` live stdout, stop live `followup_message` and wake tick/loop/budget prompts, and persisted validation output. Redaction covers `key=value` / `--flag value` / JSON `"key":"value"` forms for password/token/api-key/secret/authorization/client-secret/private-key names, `Bearer`/`Authorization: Basic` headers, AWS access-key IDs, JWTs, GitHub (`ghp_`/`gho_`/`ghu_`/`ghs_`/`ghr_`/`github_pat_`), OpenAI/Anthropic (`sk-`/`sk-proj-`/`sk-ant-`), Slack (`xox[baprs]-`), npm (`npm_`), and GitLab (`glpat-`) token formats, PEM-style `-----BEGIN … PRIVATE KEY-----` blocks (whole block replaced regardless of length), and `scheme://user:password@host` userinfo in credentialed URLs / connection strings (e.g. `postgres://`, `mongodb(+srv)://`, `redis://`, `DATABASE_URL`-style values) — this is a heuristic best-effort list, not an exhaustive secret scanner. Live stop/wake prompts keep a usable condition after scrubbing; `last-stop-response.json` strips condition payloads after `Goal:` / `toward:` markers for disk. Validation subprocesses receive a scrubbed environment (PATH/home/locale/`VIRTUAL_ENV`/Windows `APPDATA`/`LOCALAPPDATA`/common toolchain homes/`CURSOR_GOAL_*` — not ambient API keys, not privilege toggles `CURSOR_GOAL_SKIP_ACL` / `ALLOW_ANY_WORKDIR` / `ALLOW_DEAD_WAKE`, and not `PYTHONPATH`/`PYTHONHOME`/`NODE_PATH`/`NPM_CONFIG_USERCONFIG`/`MAVEN_OPTS`/`SBT_OPTS`). On Windows, shell mode pins `COMSPEC` to `%SystemRoot%\System32\cmd.exe` when that file exists. `CURSOR_GOAL_LOG_SECRETS=1` may log full commands at DEBUG.
- `last-stop-response.json` stores a redacted payload with private file mode when the OS allows. Optional durable logs: `CURSOR_GOAL_LOG_FILE=1` (sentinel → data-dir log) or an **absolute** custom path (relative paths are rejected, same policy as `CURSOR_GOAL_DATA`); symlink/reparse parents are refused before open. When set without `CURSOR_GOAL_LOG`, log level defaults to INFO.
- Stop insecure/ACL refuse emits `{}` (fail-open). Dual-hook singleflight lock miss exits silently (no stdout) so a loser cannot overwrite a real followup.
- Wake kill verifies process cmdline (Windows CIM / Unix `/proc` or `ps`) before signaling and requires a matching ownership token in `wake.pid`. Tokenless / plain-int `wake.pid` files are cleared without kill; if the PID is still alive the harness writes `wake.orphan` and `manage doctor` hard-fails until resolved. Ownership matching requires wake-loop identity (not bare `cursor-goal` / `run_goal.py` path substrings).
- When `CURSOR_GOAL_PYTHON` is set (marketplace/classic Windows `.cmd` launchers **and** Unix `wake_loop.sh`), it must be an absolute path to Python 3.12+. Classic and marketplace `.cmd` launchers reject unsafe cmd metacharacters (`&|<>^`) in `CURSOR_GOAL_PYTHON` (note: `echo %CGP%| findstr` can still expand nested `%VAR%` sequences — prefer a clean absolute path without percent signs).
- When `CURSOR_GOAL_DATA` is set, it must be an absolute path (same policy as `CURSOR_GOAL_HOME`).
- `--workdir` must not be a symlink/junction/reparse and, unless `CURSOR_GOAL_ALLOW_ANY_WORKDIR=1`, must stay under the create-time process cwd. `update_goal_fields(workdir=...)` re-checks with `assert_workdir_usable` (missing dir / symlink; not the create-time cwd jail). `eval validate` fails closed if a configured workdir is missing or insecure.
- `manage doctor` reports insecure dirs, Windows ACL harden failures (force re-harden each doctor run), orphan wake markers, stale baked Python in `.cmd` launchers, unsafe `CURSOR_GOAL_PYTHON` metacharacters, installed skill `VERSION` vs package version, PATH vs absolute Python, hook presence, classic+marketplace stacking (hard fail; scans `~/.cursor/plugins/cache/**` and `local/**`), wake health including ownership verification (with exact wake-loop command; hard fail while pursuing without live wake), fail-open continue counter, and shell mode. Installers exit non-zero when doctor hard-fails and **do not roll back** a completed file copy (the previous snapshot remains under `~/.cursor-goal/backups/`; hook-merge failure still rolls back). Classic Windows install hard-fails when data-dir ACL harden soft-fails.
