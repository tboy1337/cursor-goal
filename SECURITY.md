...
- Prefer argv-safe `--test` commands. New goals default to `shell_ok=false`; pass `--allow-shell` to permit `shell=True` (`COMSPEC`/cmd on Windows) when argv-splitting fails. `--deny-shell` and `CURSOR_GOAL_DENY_SHELL=1` remain available.
...
- Secret-ish tokens in validation commands **and validation output** / status reasons are redacted in logs/status/prompts/persisted state (heuristic; incomplete by design). Live stop `followup_message` and wake prompts keep a usable condition after heuristic secret scrubbing; `last-stop-response.json` strips condition payloads after `Goal:` / `toward:` markers for disk. Validation subprocesses receive a scrubbed environment (PATH/home/locale/`VIRTUAL_ENV`/Windows `APPDATA`/`LOCALAPPDATA`/common toolchain homes/`CURSOR_GOAL_*` — not ambient API keys, and not `PYTHONPATH`/`PYTHONHOME`/`NODE_PATH`/`NPM_CONFIG_USERCONFIG`/`MAVEN_OPTS`/`SBT_OPTS`). `CURSOR_GOAL_LOG_SECRETS=1` may log full commands at DEBUG.
