...
`/goal` is an OpenAI Codex feature (`codex-rs/ext/goal`). This package is a Cursor port of that loop, invoked as **`/cursor-goal`**. Cursor also ships a first-party `/goal` command (`CreateGoal` / `UpdateGoal`); this harness does **not** intercept it. On `/cursor-goal`, native tools own keep-going when they exist; CLEAR+YES then `manage done` then `UpdateGoal complete` owns proof of done. Worker `stop` followups and wake turn **off** once native continuation is recorded (`manage create --native` or `manage native-on`). `subagentStop` for auditor/evaluator stays. If CreateGoal is missing or fails, continuation remains `stop` / `subagentStop` plus optional wake. `CURSOR_GOAL_NATIVE=0` is a debug/test escape hatch that forces that fallback even when CreateGoal exists. Differences that matter in practice: Cursor has no Codex-style hidden idle injection or per-token goal budget; completion is gated by a separate auditor + evaluator rather than a same-model self-audit. Native `UpdateGoal` has no `blocked` / pause from the agent — on harness `blocked` or `budget-limited`, the user must pause the native goal.
