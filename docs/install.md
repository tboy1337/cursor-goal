...
| Path | Purpose |
|------|---------|
| `~/.cursor/skills/cursor-goal/SKILL.md` | Agent skill protocol |
| `~/.cursor/skills/cursor-goal/cursor_goal/` | Python package (vendored) |
| `~/.cursor/skills/cursor-goal/scripts/run_goal.py` | CLI entry for agents |
...
Package version **5.1.7** pins the clone branch below. Use it when tag `v5.1.7` exists on [GitHub Releases](https://github.com/tboy1337/cursor-goal/releases). If `git clone --branch` fails, clone `main` with the Quick install steps above ([release.md](release.md)).

```bash
git clone --branch v5.1.7 https://github.com/tboy1337/cursor-goal.git
cd cursor-goal
...
py -3 -u "$env:USERPROFILE\.cursor\skills\cursor-goal\scripts\run_goal.py" eval audit-spawn-config
...
Resolve harness commands with `manage harness-cmd` (works from `${CURSOR_PLUGIN_ROOT}/skills/cursor-goal` without a classic install). Do **not** stack classic installer hooks with marketplace hooks — pick one path. `manage doctor` **FAIL**s when both look configured.

### Three `stop_hook.cmd` roles (do not conflate)

| Variant | Where | Role |
|---------|-------|------|
| Source PATH template | `.cursor/skills/cursor-goal/scripts/stop_hook.cmd` | PATH/`CURSOR_GOAL_PYTHON` discovery; not what classic Windows install leaves on disk |
| Classic install bake | Written by `install-goal.ps1` → `~/.cursor/skills/cursor-goal/scripts/stop_hook.cmd` | Absolute Python + `stop_hook.py` (re-run installer after moving Python) |
...
**Individuals** should still use the clone + installer path above. Plugin import does not replace `install-goal.*` for classic `~/.cursor/skills/cursor-goal` installs.
