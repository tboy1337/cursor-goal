...
def test_eval_parse_result_at_missing_file(goal_home: Path) -> None:
    code, _out, err = run_cli("eval", "parse-result", "@/no/such/file.txt")
    assert code == 1
...
def test_parse_result_oversize_file(goal_home: Path, tmp_path: Path) -> None:
    from cursor_goal import evaluate as evaluate_mod
...
def test_manage_status_lock_timeout(
    ...
    monkeypatch.setattr(manage_mod, "load_goal", boom)
